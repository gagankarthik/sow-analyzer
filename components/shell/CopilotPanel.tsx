      ...
      <SheetContent side="right" className="w-full sm:max-w-[420px] p-0 flex flex-col gap-0 rounded-l-xl border-l border-border focus-visible:ring-2 focus-visible:ring-ring/50">
        <SheetHeader className="px-4 h-14 flex-row items-center justify-between border-b border-border">
          <div className="flex items-center gap-2 min-w-0">
            <span className="relative inline-flex h-7 w-7 items-center justify-center shrink-0" aria-label="Sonar">
            ...
            <div className="min-w-0">
              <SheetTitle className="text-[13px] font-semibold tracking-tight">Sonar · AI assistant</SheetTitle>
        ...
        <div ref={threadRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {busy && (
            <div className="flex flex-col gap-1">
              <div className="eyebrow text-[var(--ai-ink)] flex items-center gap-1.5"><span className="ai-pulse" />Sonar · thinking</div>
        ...
        <div className="border-t border-border p-3">
          ...
          <p className="mt-1.5 text-[10px] text-muted-foreground">Sonar answers only from this contract's clauses and cites them. It can be wrong — verify before acting.</p>
        </div>
      </SheetContent>
    </Sheet>
  );
...
function Message({ msg }: { msg: Msg }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="eyebrow text-[var(--ai-ink)] flex items-center gap-1.5"><span className="ai-pulse" />Sonar</div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {dedupeCitations(msg.citations).map((c, i) => (
                <span key={i} className="inline-flex items-center gap-1 rounded-full border border-[var(--ai-border)] bg-card px-2 py-0.5 text-[10.5px]">
                  ...
                  {c.category && <span className="text-muted-foreground">{categoryLabel(c.category)}</span>}
                </span>
              ))}
            </div>
          </>
